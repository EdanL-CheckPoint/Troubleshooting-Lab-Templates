...
#Create Cluster
mgmt_cli add simple-cluster name "AzureLab1" version "$GVERSION" ip-address "$CLUSTERIP" cluster-mode "cluster-xl-ha" firewall true vpn false members.1.name "AzureLab1gw1" members.1.one-time-password "Checkpoint123" members.1.ip-address "$MEMBER1" members.2.name "AzureLab1gw2" members.2.one-time-password "Checkpoint123" members.2.ip-address "$MEMBER2" -s id.txt --format json
